...
def scipy_optimize():
    ...
    parser.add_argument("-g", "--gaitsym_executable", default="gaitsym_2019", help="GaitSym executable [gaitsym_2019]")
    ...
    gaitsym_path = shutil.which(args.gaitsym_executable)
    if not gaitsym_path:
        print('"%s" not found' % (args.gaitsym_executable))
        sys.exit(1)
    ...
    extra_args.append(args.gaitsym_executable)
...
def evaluate_fitness(x, args):
    ...
    if verbose: print('Created "%s"' % (file_name))
    command = [gaitsym_executable, '--config', file_name]
    if verbose: print(command)
    output = subprocess.run(command, capture_output = True, timeout=600, universal_newlines=True)
    if len(output_filename) == 0:
        os.unlink(file_name)
        if verbose: print('Deleted "%s"' % (file_name))
    stdout_stderr = output.stdout + output.stderr
    if verbose: print(stdout_stderr)
    coded_values = colon_defined_numbers(stdout_stderr)
    if verbose: print(coded_values)
    if not 'Score:' in coded_values:
        print("Status : FAIL Score missing")
        sys.exit()
    if negate_score:
        score = -float(coded_values['Score:'])
    else:
        score = float(coded_values['Score:'])
...
def colon_defined_numbers(string):
    output_names  = []
    output_values = []
    tokens = string.split()
    value_next = False
    ...
    for token in tokens:
        if value_next:
            ...
            value_next = False
            continue
        current_name_list.append(token)
        if token.endswith(':'):
            output_names.append(' '.join(current_name_list))
            current_name_list = []
            value_next = True
    coded_values = {}
    for i in range(0, min(len(output_names), len(output_values))):
        coded_values[output_names[i]] = output_values[i]
    return coded_values
