...
int ReadModel(void *userData)
{
    ...
    if (gConfigFilenamePtr)
    {
        ...
        else
        {
            ...
            std::string buf;
            gXMLConverter.GetFormattedXML(&buf);
            myFile.SetRawData(buf.data(), buf.size());
            delete [] data;
        }
    }
    else
    {
        ...
        {
            gXMLConverter.LoadBaseXMLString(data + 2 * sizeof(int), strlen(data + 2 * sizeof(int)));
            delete [] data;
            return 1;
        }
    ...
    else
    {
        ...
        std::string buf;
        gXMLConverter.GetFormattedXML(&buf);
        myFile.SetRawData(buf.c_str(), buf.size());
        delete [] data;
    }
