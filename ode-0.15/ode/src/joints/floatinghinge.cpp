...
dxJointFloatingHinge::dxJointFloatingHinge( dxWorld *w ) :
        dxJoint( w )
{
    // wis // dSetZero( anchor1, 4 );
    // wis // dSetZero( anchor2, 4 );
    dSetZero( axis1, 4 );
    axis1[0] = 1;
    dSetZero( axis2, 4 );
    axis2[0] = 1;
...
void 
dxJointFloatingHinge::getSureMaxInfo( SureMaxInfo* info )
{
    info->max_m = 6;
}
...
void
dxJointFloatingHinge::getInfo1( dxJoint::Info1 *info )
{
    info->nub = 5;
...
void dxJointFloatingHinge::getInfo2( dReal worldFPS, dReal worldERP,
    int rowskip, dReal *J1, dReal *J2,
    int pairskip, dReal *pairRhsCfm, dReal *pairLoHi,
    int *findex )
{
    // wis
    // this is the linear bit that I don't need
    // set the three ball-and-socket rows
    // setBall( this, worldFPS, worldERP, rowskip, J1, J2, pairskip, pairRhsCfm, anchor1, anchor2 );
    // the rotational bit should be as before
    // ~wis
...
void dJointSetFloatingHingeAnchor( dJointID j, dReal x, dReal y, dReal z )
{
    dxJointFloatingHinge* joint = ( dxJointFloatingHinge* )j;
    dUASSERT( joint, "bad joint argument" );
    checktype( joint, FloatingHinge );
    setAnchors( joint, x, y, z, joint->anchor1, joint->anchor2 );
    joint->computeInitialRelativeRotation();
}


void dJointSetFloatingHingeAnchorDelta( dJointID j, dReal x, dReal y, dReal z, dReal dx, dReal dy, dReal dz )
{
    dxJointFloatingHinge* joint = ( dxJointFloatingHinge* )j;
    dUASSERT( joint, "bad joint argument" );
    checktype( joint, FloatingHinge );
...
void dJointSetFloatingHingeAxis( dJointID j, dReal x, dReal y, dReal z )
{
    dxJointFloatingHinge* joint = ( dxJointFloatingHinge* )j;
    dUASSERT( joint, "bad joint argument" );
    checktype( joint, FloatingHinge );
...
void dJointSetFloatingHingeAxisOffset( dJointID j, dReal x, dReal y, dReal z, dReal dangle )
{
    dxJointFloatingHinge* joint = ( dxJointFloatingHinge* )j;
    dUASSERT( joint, "bad joint argument" );
    checktype( joint, FloatingHinge );
...
void dJointGetFloatingHingeAnchor( dJointID j, dVector3 result )
{
    dxJointFloatingHinge* joint = ( dxJointFloatingHinge* )j;
    dUASSERT( joint, "bad joint argument" );
...
void dJointGetFloatingHingeAnchor2( dJointID j, dVector3 result )
{
    dxJointFloatingHinge* joint = ( dxJointFloatingHinge* )j;
    dUASSERT( joint, "bad joint argument" );
    dUASSERT( result, "bad result argument" );
...
void dJointGetFloatingHingeAxis( dJointID j, dVector3 result )
{
    dxJointFloatingHinge* joint = ( dxJointFloatingHinge* )j;
    dUASSERT( joint, "bad joint argument" );
    dUASSERT( result, "bad result argument" );
...
void dJointSetFloatingHingeParam( dJointID j, int parameter, dReal value )
{
    dxJointFloatingHinge* joint = ( dxJointFloatingHinge* )j;
    dUASSERT( joint, "bad joint argument" );
    checktype( joint, FloatingHinge );
...
dReal dJointGetFloatingHingeParam( dJointID j, int parameter )
{
    dxJointFloatingHinge* joint = ( dxJointFloatingHinge* )j;
    dUASSERT( joint, "bad joint argument" );
    checktype( joint, FloatingHinge );
...
dReal dJointGetFloatingHingeAngle( dJointID j )
{
    dxJointFloatingHinge* joint = ( dxJointFloatingHinge* )j;
    dAASSERT( joint );
    checktype( joint, FloatingHinge );
...
dReal dJointGetFloatingHingeAngleRate( dJointID j )
{
    dxJointFloatingHinge* joint = ( dxJointFloatingHinge* )j;
    dAASSERT( joint );
    checktype( joint, FloatingHinge );
...
void dJointAddFloatingHingeTorque( dJointID j, dReal torque )
{
    dxJointFloatingHinge* joint = ( dxJointFloatingHinge* )j;
    dVector3 axis;
    dAASSERT( joint );
...
dJointType
dxJointFloatingHinge::type() const
{
    return dJointTypeFloatingHinge;
}



size_t
dxJointFloatingHinge::size() const
{
    return sizeof( *this );
}
...
void
dxJointFloatingHinge::setRelativeValues()
{
    dVector3 vec;
    dJointGetFloatingHingeAnchor(this, vec);
    setAnchors( this, vec[0], vec[1], vec[2], anchor1, anchor2 );

    dJointGetFloatingHingeAxis(this, vec);
...
void
dxJointFloatingHinge::computeInitialRelativeRotation()
{
    if ( node[0].body )
    {
        if ( node[1].body )
