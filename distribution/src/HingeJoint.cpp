...
void HingeJoint::LateInitialisation()
{
...
void HingeJoint::SetStopTorqueWindow(int window)
{
    ...
    if (window > 1)
    {
        m_axisTorqueList = new double[size_t(window)](); // () initialises the array to zero
        m_axisTorqueWindow = window;
    }
    else
    {
        m_axisTorqueList = nullptr;
...
std::string *HingeJoint::CreateFromAttributes()
{
    ...
    if (GetAttribute("LowStop"s, &buf) == nullptr) return lastErrorPtr();
    double loStop = GSUtil::GetAngle(buf);
    if (GetAttribute("HighStop"s, &buf) == nullptr) return lastErrorPtr();
    double hiStop = GSUtil::GetAngle(buf);
    if (loStop >= hiStop)
    {
        setLastError("Hinge ID=\""s + GetName() +"\" LowStop >= HighStop"s);
        return lastErrorPtr();
    }
    this->SetJointStops(loStop, hiStop);

    if (GetAttribute("HighStopTorqueLimit"s, &buf))
    {
        double hiStopTorqueLimit = GSUtil::Double(buf);
        if (GetAttribute("LowStopTorqueLimit"s, &buf) == nullptr) return lastErrorPtr();
        double loStopTorqueLimit = GSUtil::Double(buf);
        this->SetTorqueLimits(loStopTorqueLimit, hiStopTorqueLimit);
        if (GetAttribute("StopTorqueWindow"s, &buf) == nullptr) return lastErrorPtr();
        this->SetStopTorqueWindow(GSUtil::Int(buf));
    }

    if (GetAttribute("StopCFM"s, &buf)) this->SetStopCFM(GSUtil::Double(buf));
    if (GetAttribute("StopERP"s, &buf)) this->SetStopERP(GSUtil::Double(buf));
    if (GetAttribute("StopBounce"s, &buf)) this->SetStopBounce(GSUtil::Double(buf));
...
void HingeJoint::Dump()
{
    ...
    if (firstDump())
    {
        ...
        {
            if (GetName().size() == 0) std::cerr << "HingeJoint::Dump error: can only dump a named object\n";
        ...
        if (dumpStream())
        {
            *dumpStream() << "Time\tXP\tYP\tZP\tXP2\tYP2\tZP2\tXA\tYA\tZA\tAngle\tAngleRate\tFX1\tFY1\tFZ1\tTX1\tTY1\tTZ1\tFX2\tFY2\tFZ2\tTX2\tTY2\tTZ2\tStopTorque\n";
        }
    }
