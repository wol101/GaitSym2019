...
//        N is the number of gas molecules (or the Avogadro constant times the amount of substance N A n {\displaystyle N_{\text{A}}n} {\displaystyle N_{\text{A}}n}),
...
std::string *FluidSacIdealGas::CreateFromAttributes()
{
    ...
    if (GetAttribute("AmountOfSubstance"s, &buf) == nullptr) return lastErrorPtr();
    this->setAmountOfSubstance(GSUtil::Double(buf));
    if (GetAttribute("ExternalPressure"s, &buf) == nullptr) return lastErrorPtr();
    this->setExternalPressure(GSUtil::Double(buf));
    if (GetAttribute("Temperature"s, &buf) == nullptr) return lastErrorPtr();
