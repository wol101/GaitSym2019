...
std::string *FixedJoint::CreateFromAttributes()
{
    ...
    if (GetAttribute("StressCalculationType"s, &buf) == nullptr) return lastErrorPtr();
    ...
    else { setLastError("Joint ID=\""s + GetName() +"\" unrecognised StressCalculationTypes"s); return lastErrorPtr(); }

    if (m_stressCalculationType != FixedJoint::none)
    {
        if (GetAttribute("LowPassType"s, &buf) == nullptr) return lastErrorPtr();
        ...
        else { setLastError("Joint ID=\""s + GetName() +"\" unrecognised LowPassType"s); return lastErrorPtr(); }

        if (GetAttribute("StressLimit"s, &buf) == nullptr) return lastErrorPtr();
        ...
        case FixedJoint::Butterworth2ndOrderLowPass:
            if (GetAttribute("CutoffFrequency"s, &buf) == nullptr) return lastErrorPtr();
            this->SetCutoffFrequency(GSUtil::Double(buf.c_str()));
            break;
        case FixedJoint::MovingAverageLowPass:
            if (GetAttribute("Window"s, &buf) == nullptr) return lastErrorPtr();
            this->SetWindow(GSUtil::Int(buf.c_str()));
            break;
        }

        double doubleList[2];
        if (GetAttribute("StressBitmapPixelSize"s, &buf) == nullptr) return lastErrorPtr();
        GSUtil::Double(buf.c_str(), 2, doubleList);
        double dx = doubleList[0];
        double dy = doubleList[1];
        if (GetAttribute("StressBitmapDimensions"s, &buf) == nullptr) return lastErrorPtr();
        GSUtil::Double(buf.c_str(), 2, doubleList);
        int nx = int(doubleList[0] + 0.5);
        int ny = int(doubleList[1] + 0.5);
        if (GetAttribute("StressBitmap"s, &buf) == nullptr) return lastErrorPtr();
...
void FixedJoint::Dump()
{
    ...
    if (firstDump())
    {
        ...
        {
            if (GetName().size() == 0) std::cerr << "FixedJoint::Dump error: can only dump a named object\n";
        ...
        if (dumpStream())
        {
            if (m_stiffness == 0)
            {
                *dumpStream() << "Time\tXP\tYP\tZP\tFX1\tFY1\tFZ1\tTX1\tTY1\tTZ1\tFX2\tFY2\tFZ2\tTX2\tTY2\tTZ2\n";
            }
            else
            {
                if (m_stressCalculationType == beam)
                {
                    *dumpStream() << "XOrigin\tYOrigin\tArea\tIx\tIy\tIxy\n";
                    *dumpStream() << m_xOrigin << "\t" << m_yOrigin << "\t" << m_area <<
                                     "\t" << m_Ix << "\t" << m_Iy << "\t" << m_Ixy <<
                                     "\n";
                    *dumpStream() << "Time\tXP\tYP\tZP\tFX1\tFY1\tFZ1\tTX1\tTY1\tTZ1\tFXS1\tFYS1\tFZS1\tTXS1\tTYS1\tTZS1\tMinStress\tMaxStress\tLowPassMinStress\tLowPassMaxStress\n";
                }
                else if (m_stressCalculationType == spring)
                {
                    *dumpStream() << "XOrigin\tYOrigin\tArea\tIx\tIy\tIxy\n";
                    *dumpStream() << m_xOrigin << "\t" << m_yOrigin << "\t" << m_area <<
                                     "\t" << m_Ix << "\t" << m_Iy << "\t" << m_Ixy <<
                                     "\n";
                    *dumpStream() << "Time\tXP\tYP\tZP\tFX1\tFY1\tFZ1\tTX1\tTY1\tTZ1\tFXS1\tFYS1\tFZS1\tTXSA1\tTYSA1\tTZSA1\tTorqueScalar\tMinStress\tMaxStress\tLowPassMinStress\tLowPassMaxStress\n";
                }
    ...
    if (dumpStream())
    {
        ...
        else
        {
            if (m_stressCalculationType == beam)
            {
                ...
                *dumpStream() << simulation()->GetTime() << "\t" << r[0] << "\t" << r[1] << "\t" << r[2] << "\t" <<
                                 JointFeedback()->f1[0] << "\t" << JointFeedback()->f1[1] << "\t" << JointFeedback()->f1[2] << "\t" <<
                                 JointFeedback()->t1[0] << "\t" << JointFeedback()->t1[1] << "\t" << JointFeedback()->t1[2] << "\t" <<
...
#ifdef USE_QT_IRRLICHT
            MainWindow *mainWindow = simulation()->GetMainWindow();
            if ((mainWindow->GetStepCount() % mainWindow->GetPreferences()->valueInt("MovieSkip")) == 0)
            {
                ...
                sprintf(pathname, "%s%08d.tif", m_Name.c_str(), dump()Count);
