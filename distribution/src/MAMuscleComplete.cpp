    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (i > nInc)
                {
                    std::cerr << "MAMuscleComplete::SetActivation Error: Unable to solve lpe " << GetName() << "\n";
...
std::string *MAMuscleComplete::CreateFromAttributes()
{
    ...
    if (GetAttribute("Width"s, &buf) == nullptr) return lastErrorPtr();
    m_width = GSUtil::Double(buf);
    this->SetMuscleProperties(vMax, f0, m_activationK, m_width);

    if (GetAttribute("TendonLength"s, &buf) == nullptr) return lastErrorPtr();
    m_tendonLength = GSUtil::Double(buf);
    if (GetAttribute("SerialStrainAtFmax"s, &buf) == nullptr) return lastErrorPtr();
    double serialStrainAtFmax = GSUtil::Double(buf);
    if (GetAttribute("SerialStrainRateAtFmax"s, &buf) == nullptr) return lastErrorPtr();
    double serialStrainRateAtFmax = GSUtil::Double(buf);
    if (GetAttribute("SerialStrainModel"s, &buf) == nullptr) return lastErrorPtr();
    ...
    else { setLastError("MUSCLE ID=\""s + GetName() + "\": Invalid SerialStrainModel"s); return lastErrorPtr(); }
    if (GetAttribute("ParallelStrainAtFmax"s, &buf) == nullptr) return lastErrorPtr();
    double parallelStrainAtFmax = GSUtil::Double(buf);
    if (GetAttribute("ParallelStrainRateAtFmax"s, &buf) == nullptr) return lastErrorPtr();
    double parallelStrainRateAtFmax = GSUtil::Double(buf);
    if (GetAttribute("ParallelStrainModel"s, &buf) == nullptr) return lastErrorPtr();
    ...
    else { setLastError("MUSCLE ID=\""s + GetName() + "\": Invalid ParallelStrainModel"s); return lastErrorPtr(); }
    ...
    if (GetAttribute("ActivationKinetics"s, &buf) == nullptr) return lastErrorPtr();
    bool activationKinetics = GSUtil::Bool(buf);
    if (activationKinetics)
    {
        if (GetAttribute("FastTwitchProportion"s, &buf) == nullptr) return lastErrorPtr();
        m_akFastTwitchProportion = GSUtil::Double(buf);
        if (GetAttribute("TActivationA"s, &buf) == nullptr) return lastErrorPtr();
        m_akTActivationA = GSUtil::Double(buf);
        if (GetAttribute("TActivationB"s, &buf) == nullptr) return lastErrorPtr();
        m_akTActivationB = GSUtil::Double(buf);
        if (GetAttribute("TDeactivationA"s, &buf) == nullptr) return lastErrorPtr();
        m_akTDeactivationA = GSUtil::Double(buf);
        if (GetAttribute("TDeactivationB"s, &buf) == nullptr) return lastErrorPtr();
    ...
    if (GetAttribute("InitialFibreLength"s, &buf) == nullptr) return lastErrorPtr();
    m_initialFibreLength = GSUtil::Double(buf);
    this->SetInitialFibreLength(m_initialFibreLength);
    if (GetAttribute("ActivationRate"s, &buf) == nullptr) return lastErrorPtr();
    this->SetActivationRate(GSUtil::Double(buf));
    if (GetAttribute("StartActivation"s, &buf) == nullptr) return lastErrorPtr();
    m_startActivation = GSUtil::Double(buf);
    this->SetStartActivation(m_startActivation);
    if (GetAttribute("MinimumActivation"s, &buf) == nullptr) return lastErrorPtr();
...
void MAMuscleComplete::Dump()
{
    ...
    if (firstDump())
    {
        ...
        {
            if (GetName().size() == 0) std::cerr << "MAMuscleComplete::Dump error: can only dump a named object\n";
        ...
        if (dumpStream())
        {
            *dumpStream() << "Time\tm_Stim\talpha\tlen\tv\tlastlpe\tfce\tlpe\tfpe\tlse\tfse\tvce\tvse\ttargetFce\tf0\terr\tESE\tEPE\tPSE\tPPE\tPCE\ttension\tlength\tvelocity\tPMECH\tPMET\n";
        }
    }
