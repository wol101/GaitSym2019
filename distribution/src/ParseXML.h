...
public:
    ...
    std::string *LoadModel(const char *buffer, size_t length, const char *rootNode);
    std::string SaveModel();

    std::vector<std::unique_ptr<XMLElement> > *elementList();

private:
    rapidxml::xml_attribute<char> *CreateXMLAttribute(rapidxml::xml_node<char> *cur, const std::string &name, const std::string &newValue);
