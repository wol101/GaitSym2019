...
public:
    ...
    inline  Vector Normalize(Vector u);
    ...
    inline Vector::Vector(const double *d)
    {
        x = d[0];
        y = d[1];
        z = d[2];
    ...
    inline  double Vector::Magnitude(void) const
    {
        return std::sqrt(x*x + y*y + z*z);
    }

    inline  double Vector::Magnitude2(void) const
    {
        return (x*x + y*y + z*z);
    }
    ...
    inline const double *Vector::constData(void) const
    {
        return &x;
    }
    ...
    inline  Vector Vector::Cross(Vector v) const
    {
    ...
    inline double Vector::Dot(Vector v) const
    {
...
public:
        ...
        void Normalize(void);
        Quaternion Conjugate(void) const { return Quaternion(n, -v.x, -v.y, -v.z); }
    ...
    inline  Vector  MakeEulerAnglesFromQRadian(Quaternion q);
    inline  Quaternion MakeQFromAxisAngle(double x, double y, double z, double angle, bool fast = false);
    inline  Quaternion MakeQFromAxisAngle(const Vector &axis, double angle, bool fast = false);
    ...
    inline  Vector  QVRotate(Quaternion q, Vector v) // so called 'sandwich product'
    {
#ifdef EASY_TO_READ
        Quaternion t;
        t = q*v*(~q);
    ...
    // wis  - added routine to make a Quaternion from an axis and a rotation angle in radians
    inline  Quaternion MakeQFromAxisAngle(const Vector &axis, double angle, bool fast)
    {
        if (fast)
        {
            // could use cos(x)^2 = 1 - sin(x)^2
    ...
    inline void MakeAxisAngleFromQ(Quaternion q1, double *xa, double *ya, double *za, double *angle)
    {
        if (q1.n > 1) q1.Normalize();           // if w > 1 acos and sqrt will produce errors, this cant happen if quaternion is normalised
        *angle = 2 * std::acos(q1.n);
        double s = std::sqrt(1 - q1.n * q1.n);  // assuming quaternion normalised then w is less than 1, so term always positive.
    ...
    inline Quaternion FindRotation(Vector v1, Vector v2)
    {
        ...
        if (q.n < epsilon) // this only occurs if a 180 degree rotation is needed
        {
            ...
            q = MakeQFromAxisAngle(perp.x, perp.y, perp.z, pi);
        }
        else
        {
            q.v = v1 ^ v2;
    ...
    inline Vector Projection(Vector v, Vector u)
    {
        // (v . norm(u) ) norm(u)
        u.Normalize();
        return v.Dot(u) * u;
...
//       otherwise, outputs are both unit
    inline void SwingTwistDecomposition(const Quaternion &rotation, const Vector &direction, Quaternion *swing, Quaternion *twist)
    {
        Vector ra(rotation.v.x, rotation.v.y, rotation.v.z); // rotation axis
        Vector p = Projection(ra, direction); // return projection v1 on to v2  (parallel component)
...
public:
    ...
    inline  float Dot(Vector u, Vector v);
    ...
    inline Vector::Vector(const float *d)
    {
        x = d[0];
        y = d[1];
        z = d[2];
    ...
    inline  float Vector::Magnitude2(void) const
    {
        return (x*x + y*y + z*z);
    }
    ...
    inline float Vector::Dot(Vector v) const
    {
        return (x*v.x + y*v.y + z*v.z);
    }
...
public:
        ...
        Quaternion(void);
        Quaternion(float nn, float x, float y, float z);
        Quaternion(const float *q_nxyz);

        void Set(float nn, float x, float y, float z);
    ...
    inline Quaternion  MakeQFromEulerAnglesRadian(float roll, float pitch, float yaw);
    inline Vector  MakeEulerAnglesFromQ(Quaternion q);
    inline Vector  MakeEulerAnglesFromQRadian(Quaternion q);
    inline Quaternion  MakeQFromAxisAngle(float x, float y, float z, float angle, bool fast = false);
    inline Quaternion MakeQFromAxisAngle(const Vector &axis, float angle, bool fast = false);
    inline void MakeAxisAngleFromQ(Quaternion q1, float *xa, float *ya, float *za, float *angle);
    inline Quaternion FindRotation(Quaternion qa, Quaternion qb);
    ...
    inline  Quaternion::Quaternion(float nn, float x, float y, float z)
    {
        this->n = nn;
        this->v.x = x;
        this->v.y = y;
    ...
    inline Quaternion::Quaternion(const float *q_nxyz)
    {
        n = q_nxyz[0];
        v.x = q_nxyz[1];
        v.y = q_nxyz[2];
    ...
    inline  void Quaternion::Set(float nn, float x, float y, float z)
    {
        this->n = nn;
        this->v.x = x;
        this->v.y = y;
    ...
    // wis  - added routine to make a Quaternion from an axis and a rotation angle in radians
    inline Quaternion MakeQFromAxisAngle(float x, float y, float z, float angle, bool fast)
    {
        if (fast)
        {
            // could use cos(x)^2 = 1 - sin(x)^2
    ...
    // wis  - added routine to make a Quaternion from an axis and a rotation angle in radians
    inline  Quaternion MakeQFromAxisAngle(const Vector &axis, float angle, bool fast)
    {
        if (fast)
        {
            // could use cos(x)^2 = 1 - sin(x)^2
    ...
    inline void MakeAxisAngleFromQ(Quaternion q1, float *xa, float *ya, float *za, float *angle)
    {
        if (q1.n > 1) q1.Normalize();          // if w > 1 acos and sqrt will produce errors, this cant happen if quaternion is normalised
        *angle = 2 * std::acos(q1.n);
        float s = std::sqrt(1 - q1.n * q1.n);  // assuming quaternion normalised then w is less than 1, so term always positive.
        if (s < epsilon)                     // test to avoid divide by zero, s is always positive due to sqrt
        {
            // if s close to zero then direction of axis not important
            *angle = 0;
...
public:
        ...
        Matrix3x3(Quaternion q);
        Matrix3x3(const float *mat_r1c1r1c2r1c3_r2c1r2c2r2c3_r3c1r3c2r3c3);
    ...
    inline Matrix3x3::Matrix3x3(const float *mat_r1c1r1c2r1c3_r2c1r2c2r2c3_r3c1r3c2r3c3)
    {
        e11 = mat_r1c1r1c2r1c3_r2c1r2c2r2c3_r3c1r3c2r3c3[0];
        e12 = mat_r1c1r1c2r1c3_r2c1r2c2r2c3_r3c1r3c2r3c3[1];
        e13 = mat_r1c1r1c2r1c3_r2c1r2c2r2c3_r3c1r3c2r3c3[2];
