...
CylinderWrapStrap::CylinderWrapStrap()
{
    ...
    m_PathCoordinates = new pgd::Vector[size_t(m_NumWrapSegments) + 2];
#endif
}
...
void CylinderWrapStrap::SetCylinderBody(Body *body)
{
    ...
    if (PointForceList()->size() <= 2)
    {
        ...
        PointForceList()->push_back(cylinder);
    }
    else
    {
        PointForceList()->at(2)->body = m_CylinderBody;
    }
}
...
void CylinderWrapStrap::Calculate(double simulationTime)
{
    int i;
    ...
    m_WrapStatus = CylinderWrap(cylinderOriginPosition, cylinderInsertionPosition, m_CylinderRadius, m_NumWrapSegments, M_PI,
                                ...
                                &length, m_PathCoordinates, &m_NumPathCoordinates);
    ...
    PointForce *theCylinder = (*PointForceList())[2];
...
std::string *CylinderWrapStrap::CreateFromAttributes()
{
    ...
    if (GetAttribute("CylinderMarkerID"s, &buf) == nullptr) return lastErrorPtr();
    auto cylinderMarker = simulation()->GetMarkerList()->find(buf);
    if (cylinderMarker == simulation()->GetMarkerList()->end())
    {
        setLastError("STRAP ID=\""s + GetName() +"\" CylinderMarker not found"s);
        return lastErrorPtr();
    }
    this->SetCylinder(cylinderMarker->second);
    if (GetAttribute("CylinderRadius"s, &buf) == nullptr) return lastErrorPtr();
...
void CylinderWrapStrap::AppendToAttributes()
{
    ...
    setAttribute("CylinderMarkerID"s, m_cylinderMarker->GetName());
