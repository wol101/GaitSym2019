...
std::string *FluidSacIncompressible::CreateFromAttributes()
{
    ...
    if (GetAttribute("FluidVolume"s, &buf) == nullptr) return lastErrorPtr();
    this->setFluidVolume(GSUtil::Double(buf));
    if (GetAttribute("BulkModulus"s, &buf) == nullptr) return lastErrorPtr();
    this->setBulkModulus(GSUtil::Double(buf));
    if (GetAttribute("StartingPressure"s, &buf) == nullptr) return lastErrorPtr();
