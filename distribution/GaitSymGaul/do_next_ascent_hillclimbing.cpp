// function to perform next ascent hillclimbing
...
void do_next_ascent_hillclimbing(DataFile *config_file)
{
    int population_size;
    int chromosome_size;
    ...
    strcpy(log_filename,"NextAscentHillclimbingLog.txt");
    ...
    for (int i_ent = 0; i_ent < pop->stable_size; i_ent++)
    {
        ...
        ga_next_ascent_hillclimbing(pop,                // population      *pop,
                                    solution,           // entity           *best,
                                    hc_max_iterations   // const int        max_iterations
                                    );

        plog(LOG_NORMAL, "RESULT %d  Next Ascent Hillclimbing Fitness = %g", i_ent, solution->fitness);
