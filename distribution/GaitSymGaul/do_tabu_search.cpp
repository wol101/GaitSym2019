// function to perform a tabu search
...
double g_mutation_sd;
XMLConverter g_XMLConverter;
extern Simulation *gSimulation;

void do_tabu_search(DataFile *config_file)
{
    int population_size;
    int chromosome_size;
    char model_config_file[512];
    int tabu_list_length;
    int tabu_neighbourhood_search_count;
    int tabu_max_iterations;
    ...
    try
    {
        ...
        THROWIF(config_file->RetrieveParameter("tabu_list_length", &tabu_list_length));
        THROWIF(config_file->RetrieveParameter("tabu_neighbourhood_search_count", &tabu_neighbourhood_search_count));
        THROWIF(config_file->RetrieveParameter("tabu_max_iterations", &tabu_max_iterations));
    ...
    strcpy(log_filename,"TabuLog.txt");
    ...
    ga_population_set_tabu_parameters(
            pop,                                /* population           *pop */
            ga_tabu_check_double,               /* GAtabu_accept        tabu acceptance criterion */
            tabu_list_length,                   /* const int            tabu list length */
            tabu_neighbourhood_search_count     /* const int            tabu neighbourhood search count */
            );
    ...
    for (int i_ent = 0; i_ent < pop->stable_size; i_ent++)
    {
        ...
        ga_tabu(
                ...
                tabu_max_iterations         /* const int            max_iterations */
                );

        plog(LOG_NORMAL, "RESULT %d Tabu Fitness = %g", i_ent, solution->fitness);
