...
FacetedSphere::FacetedSphere(double radius, int maxlevels, const QColor &colour,
...
void FacetedSphere::WritePOVRay(std::ostringstream &theString)
{
    theString << "object {\n";
    theString << "  sphere {\n";
    theString << "    <" << GetDisplayPosition()[0] << "," << GetDisplayPosition()[1] << "," <<
              GetDisplayPosition()[2] << ">, " << m_Radius << "\n";

    // now colour
    theString << "    pigment {\n";
    theString << "      color rgbf<" << GetColour().redF() << "," << GetColour().greenF() << "," <<
              GetColour().blueF() << "," << 1 - GetColour().alphaF() << ">\n";
    theString << "    }\n";
...
int FacetedSphere::EstimateLevel(int requestedFaces, int *actualFaces)
{
    ...
    int level = int(std::ceil(actualLevel));
    if (actualFaces) *actualFaces = 8 * int(std::pow(2.0, double(level)) + 0.5);
    return level;
}
