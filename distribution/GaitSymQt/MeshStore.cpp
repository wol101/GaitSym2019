...
void MeshStore::addMesh(const std::string &path, const std::vector<double> &vertexList,
...
uint64_t MeshStore::getCurrentMemory()
{
    ...
    {
        totalMemory += it->second->vertexList.size() * sizeof(double) + it->second->normalList.size() *
                       sizeof(double) + it->second->colourList.size() * sizeof(double);
        totalMemory += sizeof(MeshStoreObject);
    }
    return totalMemory;
}
...
void MeshStore::setTargetMemory(uint64_t targetMemory)
{
    while (getCurrentMemory() > targetMemory)
    {
        ...
        m_lastAccessedMapByTime.erase(it);
    }
}
