...
class SimulationWidget : public QLabel
{
    Q_OBJECT
public:
    SimulationWidget(QWidget *parent = nullptr);
    virtual ~SimulationWidget() Q_DECL_OVERRIDE;
...
//    void clearRootEntity();
    ...
    static int closestHit(const Qt3DRender::QAbstractRayCaster::Hits &hits);
    static void deleteChildrenRecursively(Qt3DCore::QNodeVector vector);
...
//    Qt3DRender::QLayer *m_pickableLayer;
    ...
    OffscreenEngine *m_offscreenEngine;
    OffscreenEngineDelegate *m_offscreenEngineDelegate;
