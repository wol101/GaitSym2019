...
void dJointGetAMotorEulerReferenceVectors( dJointID j , dVector3 ref1 , dVector3 ref2 )
{
    dxJointAMotor* joint = ( dxJointAMotor* )j;
    dAASSERT( joint );
    checktype( joint, AMotor );
    for (int i = 0; i < 3; i++)
    {
        ref1[i] = joint->reference1[i];
        ref2[i] = joint->reference2[i];
    }
}

void dJointSetAMotorEulerReferenceVectors( dJointID j, dVector3 ref1 , dVector3 ref2 )
{
    dxJointAMotor* joint = ( dxJointAMotor* )j;
    dAASSERT( joint );
    checktype( joint, AMotor );
    for (int i = 0; i < 3; i++)
    {
         joint->reference1[i] = ref1[i];
         joint->reference2[i] = ref2[i];
    }
}
